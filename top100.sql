...
CREATE DATABASE
posts=# \c peliculas
Ahora está conectado a la base de datos «peliculas» con el usuario «administrador».

--crear tabla peliculas

peliculas=# create table peliculas(id int PRIMARY KEY, Pelicula VARCHAR(100), Estreno INT, Director VARCHAR(100));
CREATE TABLE

-- importar archivos csv

peliculas=# \copy peliculas from 'C:\Users\Jorge\Desktop\Desafio top 100\Top100\peliculas.csv' delimiter ',' csv header;
COPY 100
...
peliculas=# create table reparto(id int, actor VARCHAR(100), foreign key(id) references peliculas(id));
CREATE TABLE
peliculas=# \copy reparto from 'C:\Users\Jorge\Desktop\Desafio top 100\Top100\reparto.csv' delimiter ',';
COPY 1051
...
--listar todos los actores de titanic

peliculas=# SELECT actor FROM reparto WHERE id = '2';
       actor
-------------------
 Leonardo DiCaprio
 Kate Winslet
...
-- contar peliculas Harrison Ford 

peliculas=# SELECT count(id) as "cantidad de peliculas" FROM reparto where actor = 'Harrison Ford';
 cantidad de peliculas
-----------------------
                     8
(1 fila)

--peliculas estrenadas entre 1990 y 1999

peliculas=# SELECT * FROM peliculas WHERE estreno between 1990 and 1999;
 id |                  pelicula                  | estreno |       director
----+--------------------------------------------+---------+-----------------------
  1 | Forest Gump                                |    1994 | Robert Zemeckis
  2 | Titanic                                    |    1997 | James Cameron
...
(100 filas)
'
--consultar titulo mas grande

peliculas=# SELECT pelicula, length(pelicula) as "longitud titulo" FROM peliculas WHERE LENGTH(pelicula)=( SELECT MAX(length(pelicula)) FROM peliculas);
                       pelicula                        | longitud titulo
-------------------------------------------------------+-----------------
 Sweeney Todd: El barbero diab├│lico de la calle Fleet |              53
(1 fila)
